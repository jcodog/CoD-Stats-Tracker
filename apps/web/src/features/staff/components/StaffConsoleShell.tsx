      ...
      <SidebarInset className="min-h-0 overflow-hidden bg-background md:peer-data-[variant=inset]:m-1 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-1">
        ...
        <div className="flex min-h-0 flex-1 flex-col px-4 py-6 md:px-6 md:py-8 lg:px-8">
          <main
            ...
            tabIndex={-1}
          >
            <div className="mx-auto flex min-h-full w-full max-w-350 flex-col gap-8 [&_[data-slot=card-content]]:px-4 [&_[data-slot=card-content]]:py-4 md:[&_[data-slot=card-content]]:px-5 md:[&_[data-slot=card-content]]:py-5 [&_[data-slot=card-footer]]:border-border/60 [&_[data-slot=card-footer]]:bg-transparent [&_[data-slot=card-header]]:rounded-none [&_[data-slot=card-header]]:border-b [&_[data-slot=card-header]]:border-border/60 [&_[data-slot=card-header]]:px-4 [&_[data-slot=card-header]]:py-4 md:[&_[data-slot=card-header]]:px-5 md:[&_[data-slot=card-header]]:py-5 [&_[data-slot=card]]:gap-0 [&_[data-slot=card]]:rounded-lg [&_[data-slot=card]]:border [&_[data-slot=card]]:border-border/60 [&_[data-slot=card]]:bg-background [&_[data-slot=card]]:py-0 [&_[data-slot=card]]:ring-0">
              {children}
            </div>
          </main>
        </div>
