...
type StatsDashboardCtx = Pick<QueryCtx, "auth" | "db"> | Pick<MutationCtx, "auth" | "db">
...
export function getSessionDisplayTitle(session: Pick<Doc<"sessions">, "codTitle" | "titleLabelSnapshot">) {
...
export function sessionMatchesRankedConfig(args: {
  ...
  if (args.session.titleKey) {
    return normalizeStatsLookupValue(args.session.titleKey) ===
...
export async function requireAuthenticatedStatsActor(
  ...
  if (!identity) {
    throw new Error("A signed-in session is required to access dashboard stats.")
  }
  ...
  const statsUserIdCandidates = await getStatsUserIdCandidatesForIdentity(ctx, identity)
...
export async function getCurrentRankedConfig(ctx: Pick<QueryCtx | MutationCtx, "db">) {
...
export async function collectOwnedSessions(
  ...
  const [ownerSessions, ...legacySessionGroups] = await Promise.all([
    ctx.db
      .query("sessions")
      .withIndex("by_owner_startedAt", (query) => query.eq("ownerUserId", actor.user._id))
      .order("desc")
      .collect(),
    ...actor.statsUserIdCandidates.map((candidate) =>
      ctx.db.query("sessions").withIndex("by_user", (query) => query.eq("userId", candidate)).collect()
    ),
  ])
  ...
  return Array.from(dedupedSessions.values()).sort(
    (left, right) => right.startedAt - left.startedAt || right._creationTime - left._creationTime
  )
}
...
export async function getOwnedSessionGames(
  ...
  return await ctx.db
    .query("games")
    .withIndex("by_session_createdat", (query) => query.eq("sessionId", session.uuid))
    .order("asc")
    .collect()
}
