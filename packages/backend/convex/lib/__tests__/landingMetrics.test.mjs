...
describe("landing metrics helpers", () => {
  ...
  it("creates, aggregates, and deduplicates user counters", async () => {
    ...
    expect(await getUserLandingCounters(queryCtx, ["user_123", "user_123"])).toEqual({
      ...EMPTY_LANDING_COUNTERS,
      matchesIndexed: 5,
      wins: 4,
      losses: 1,
    ...
    expect(await getUserLandingCounters(queryCtx, ["user_123", "user_456"])).toEqual({
      matchesIndexed: 5,
      sessionsTracked: 2,
      activeSessions: 1,
      wins: 4,
